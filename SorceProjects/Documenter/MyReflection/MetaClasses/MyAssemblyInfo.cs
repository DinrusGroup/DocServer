...
namespace BestCS.DocNet.MyReflection.MetaClasses
{
  class MyAssemblyInfo : MetaClass, ISummarisableMember
  {
    ...
    public void ReadAssembly(string assemblyAbsolutePath, IEnumerable<string> excludedNamespaces)
    {
      ...
      {
        Logger.Error("Не удаётся загрузить содержимое сборки в сборку с другим названием.");
        return;
      }
    ...
    public void ReadXmlDocumentation(string xmlDocPath)
    {
      ...
      {
        Logger.Warning("Неверный формат файла документации XML: " + xmlDocPath);

        return;
      }

      Debug.Assert(assemblyName == name, "Не удаётся загрузить содержимое сборки в сборку с другим названием.");
