    ...
    class Program
    {
        ...
        private static void Main(string[] args)
        {
            ...
            for (int i = 0; i < filesNames.Count; i++)
            {
                ...
                {
                    if (verboseLevel > 2) { Console.WriteLine("Исключён файл {0} (исполнимый модуль программы).", fileName); }
        ...
        private static void ProcessOptions(List<string> options)
        {
            foreach (string option in options)
            {
                ...
                else if (opName == "outputdirectory" || opName == "od")
                {
                    ...
                    {
                        Console.WriteLine("Ошибка: Папка вывода не пуста.");
                        Environment.Exit(1);
                    }
        ...
        private static void ProcessFilesNames(List<string> filesNames, IEnumerable<string> excludedNamespaces)
        {
            ...
            foreach (string fullFileName in filesNames)
            {
                ...
                {
                    if (verboseLevel > 2) { Console.WriteLine("Исключён файл {0} (исключение задано в опциях).", Path.GetFileName(fullFileName)); }
            ...
            foreach (string fullFileName in filesNames)
            {
                ...
                if (!File.Exists(fullFileName))
                {
                    if (verboseLevel > 2) { Console.WriteLine("Пропущен файл документации {0} (файла не существует).", fileName); }
            ...
            foreach (string fullFileName in filesNames)
            {
                ...
                if (!File.Exists(fullFileName))
                {
                    if (verboseLevel > 2) { Console.WriteLine("Пропущен дополнительный файл документации {0} (файла не существует).", fileName); }
                ...
                {
                    if (verboseLevel > 2) { Console.WriteLine("Пропущен дополнительный файл документации {0} (неверный формат файла).", fileName); }

                    continue;
                }

                if (verboseLevel > 2) { Console.Write("Читается дополнительный файл документации {0}... ", fileName); }
        ...
        private static void PrintUsageAndExit()
        {
            ...
            Console.WriteLine(".xml и .docs из указанной папки будут обработаны (за исключением");
        ...
        {
            Debug.Assert(false, "Это невозможно!");
        }
